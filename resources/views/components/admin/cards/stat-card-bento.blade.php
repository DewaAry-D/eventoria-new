@props([
    ...
    'title',
    'badgeText' => null,
    'badgeType' => 'neutral', // success, error, warning, neutral
    'iconBg' => 'neutral'   // primary, success, error, warning, neutral
])
...
<div class="bg-surface-container-lowest p-md sm:p-lg rounded-xl border border-outline-variant/30 flex flex-row sm:flex-col justify-between items-center sm:items-start w-full min-h-[5.5rem] sm:min-h-[11rem] shadow-card hover:shadow-md hover:-translate-y-1 transition-all duration-300 ease-out group select-none">
    ...
    <div class="flex items-center sm:justify-between sm:items-start sm:w-full gap-md sm:gap-0">
        <div class="p-sm rounded-xl shrink-0 transition-transform duration-300 group-hover:scale-105
            ...
            {{ $iconBg === 'warning' ? 'bg-warning/10 text-warning' : '' }}
            {{ $iconBg === 'error' ? 'bg-error/10 text-error' : '' }}
        ...
        @if($badgeText)
            <span class="hidden sm:inline-flex text-label-md px-sm py-1 rounded-xl border font-bold items-center gap-xs tracking-wide shrink-0 transition-transform duration-300 group-hover:scale-105
                {{ $badgeType === 'success' ? 'bg-success/10 text-success border-success/10' : '' }}
                {{ $badgeType === 'warning' ? 'bg-warning/10 text-warning border-warning/10' : '' }}
                {{ $badgeType === 'error'   ? 'bg-error/10 text-error border-error/10' : '' }}
                {{ $badgeType === 'neutral' ? 'bg-surface-container-low text-secondary border-outline-variant/30' : '' }}
            ">
                @if(isset($badgeIcon))
                    {{ $badgeIcon }}
                @endif
    ...
    <div class="mt-0 sm:mt-xl flex flex-col items-end sm:items-start justify-center sm:justify-end shrink-0">
        ...
        @if($badgeText)
            <span class="inline-flex sm:hidden text-[10px] px-2 py-0.5 rounded-xl border font-bold items-center gap-[2px] tracking-wide mb-1 transition-transform duration-300 group-hover:scale-105
                {{ $badgeType === 'success' ? 'bg-success/10 text-success border-success/10' : '' }}
                {{ $badgeType === 'warning' ? 'bg-warning/10 text-warning border-warning/10' : '' }}
                {{ $badgeType === 'error'   ? 'bg-error/10 text-error border-error/10' : '' }}
                {{ $badgeType === 'neutral' ? 'bg-surface-container-low text-secondary border-outline-variant/30' : '' }}
