        ...
        <nav x-data="{ mobileMenuOpen: false }" class="bg-white border-b border-gray-200 sticky top-0 z-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16 relative">
                    ...
                    <div class="hidden md:flex space-x-8 items-center h-full absolute left-1/2 transform -translate-x-1/2">
                        ...
                        <a href="#" class="inline-flex items-center h-full px-1 pt-1 border-b-2 {{ request()->routeIs('mahasiswa.schedule') ? 'border-primary text-primary font-bold' : 'border-transparent text-on-surface-variant hover:text-primary hover:border-outline' }} text-sm font-medium transition duration-150">
                            Schedule
                        </a>
                        <a href="#" class="inline-flex items-center h-full px-1 pt-1 border-b-2 {{ request()->routeIs('mahasiswa.my-events') ? 'border-primary text-primary font-bold' : 'border-transparent text-on-surface-variant hover:text-primary hover:border-outline' }} text-sm font-medium transition duration-150">
                            My Event
                        </a>
                    </div>
            ...
            <div x-show="mobileMenuOpen" x-transition class="md:hidden border-t border-outline-variant bg-surface-container-lowest">
                <div class="px-2 pt-2 pb-3 space-y-1">
                    ...
                    <a href="#" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('mahasiswa.schedule') ? 'bg-primary-fixed text-primary font-bold' : 'text-on-surface-variant hover:bg-surface-container-low hover:text-primary' }}">
                        Schedule
                    </a>
                    <a href="#" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('mahasiswa.my-events') ? 'bg-primary-fixed text-primary font-bold' : 'text-on-surface-variant hover:bg-surface-container-low hover:text-primary' }}">
                        My Event
                    </a>
