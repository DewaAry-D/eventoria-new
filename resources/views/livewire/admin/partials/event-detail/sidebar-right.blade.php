<div class="w-full flex flex-col gap-md lg:gap-lg min-w-0">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-md lg:gap-lg items-stretch">
        
        <!-- KARTU KIRI: DETAIL PROFIL ORGANISASI -->
        <div class="p-md sm:p-lg bg-surface-container-lowest rounded-3xl border border-outline-variant/30 shadow-card flex flex-col justify-between gap-md select-none animate-fade-in">
        ...
        <!-- KARTU KANAN: RIWAYAT PENGAJUAN EVENT -->
        <div class="p-md sm:p-lg bg-surface-container-lowest rounded-3xl border border-outline-variant/30 shadow-card flex flex-col gap-sm select-none">
            ...
            @if($riwayatPengajuan && $riwayatPengajuan->count() > 0)
                <div class="w-full flex flex-col max-h-[190px] overflow-y-auto pr-xs divide-y divide-surface-container
                            [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-outline-variant/60 [&::-webkit-scrollbar-thumb]:rounded-full">
                    @foreach($riwayatPengajuan as $riwayat)
                        ...
                        <a href="{{ route('admin.event.detail', $riwayat->id) }}" wire:navigate class="py-sm first:pt-0 last:pb-0 flex items-center justify-between gap-md group/item transition-all duration-200 cursor-pointer">
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-caption text-secondary/60 font-semibold leading-none mt-1.5">
                                    {{ \Carbon\Carbon::parse($riwayat->created_at)->translatedFormat('d M Y') }}
                                </p>
                            </div>
                            <span class="px-2.5 py-0.5 border text-[10px] font-extrabold rounded-xl {{ $statusColor }}">
                                {{ $statusTeks }}
                            </span>
                        </a>
                    @endforeach
            ...
            @else
                <div class="py-xs w-full flex-1 flex items-center justify-center">
                    <p class="text-caption text-secondary/40 font-bold uppercase tracking-wider">Belum Ada Riwayat Event</p>
                </div>
            @endif
        </div>
    ...
    @if($eStatus === 'pending_approval')
        <div class="w-full bg-surface-container-low border border-outline-variant/20 rounded-3xl p-md flex flex-col sm:flex-row items-center justify-between gap-md select-none mt-xs">
            <div class="text-left hidden md:block max-w-sm">
                <span class="text-caption font-extrabold text-primary uppercase tracking-wider block mb-0.5">Verifikasi Otoritas</span>
                <p class="text-[11px] text-secondary/70 font-semibold leading-normal">
                    Pastikan detail substansi, pamflet, dan tanggal pelaksanaan event telah sesuai dengan ketentuan institusi.
                </p>
            </div>
            
            <div class="flex flex-col sm:flex-row gap-sm w-full md:w-auto justify-end shrink-0">
                <button type="button" 
                        @click="$dispatch('open-modal-reject-event', { id: {{ $event->id }}, name: '{{ addslashes($event->nama_event) }}' })"
                        class="px-lg py-2.5 rounded-xl border border-error/30 text-error hover:bg-error/5 font-bold text-body-sm flex items-center justify-center gap-xs transition-all cursor-pointer shadow-2xs w-full sm:w-auto">
                ...
                <button type="button" 
                        ...
                        class="px-xl py-2.5 rounded-xl text-on-primary bg-success hover:bg-success/90 font-bold text-body-sm flex items-center justify-center gap-xs transition-all shadow-md cursor-pointer w-full sm:w-auto">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
    ...
    <x-admin.modals.confirm-modal id="approve-event" title="Setujui Pengajuan Event" wireAction="approveEvent">
        Apakah Anda yakin ingin menerbitkan event <strong class="text-primary font-bold">"<span x-text="targetName"></span>"</strong>? Event ini akan langsung dipublikasikan ke publik.
    </x-admin.modals.confirm-modal>

    <x-admin.modals.reject-modal id="reject-event" title="Tolak Pendaftaran?" wireAction="rejectEvent" />
